#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
#   MyCoiffeur — Wireless Device Test Runner
...
APK_PATH="$SCRIPT_DIR/../MyCoiffeur.apk"
...
for _sdk in "$HOME/Library/Android/sdk" "$HOME/Android/sdk" "$HOME/.mycoiffeur-android-sdk"; do
...
echo "  ╚═════╝ ╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝   "
echo -e "${RESET}"
echo -e "  ${BOLD}MyCoiffeur — Device Test Runner${RESET}"
...
if [ "$PLATFORM" = "android" ]; then
  ...
  # ── Install APK ──────────────────────────────────────────────────
  if [ -f "$APK_PATH" ]; then
    step "Installing MyCoiffeur APK..."
...
# ════════════════════════════════════════════════════════════════════
mkdir -p "$REPORTS_DIR"
RUN_SCRIPT="/tmp/mycoiffeur_run_${REPORT_TS}.sh"
...
# Write the runner sub-script
{
  ...
  echo "echo '════════════════════════════════════════════════════════'"
  echo "echo '  MyCoiffeur Maestro — ${#FLOW_FILES[@]} flows on $DEVICE_TARGET'"
  ...
  echo "_PKG=\$(adb -s \"$DEVICE_TARGET\" shell pm list packages 2>/dev/null | grep -i "my_coiffeur" | head -1 | tr -d '\\r' || true)"
  echo "if [ -z \"\$_PKG\" ]; then"
  echo "  echo 'App     : NOT FOUND — com.example.my_coiffeur is not installed on the device'"
